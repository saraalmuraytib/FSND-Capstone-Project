...
#-------------------- POST Requests --------------------
#-------------------- PATCH Requests --------------------
@app.route("/tutor/<int:tutor_id>/appointments/<int:appointment_id>", methods=['PATCH'])
def update_appointment(tutor_id, appointment_id):
    tutor = Tutor.query.filter(Tutor.id == tutor_id).one_or_none()
    if tutor is None:
         abort(404)
    else:
      appointment = Tutor.query.filter(Appointments.appointment_id == appointment_id).one_or_none()
