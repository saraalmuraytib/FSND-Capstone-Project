...
def create_appointment(student_id):
    ...
    if student is None:
      ...
      tutor = Tutor.query.filter(Tutor.id == tutor_id).get_or_404()
      try:
        # json.dumps ---> Serialize obj to a JSON formatted str.
        new_appointment = Appointments(start_time=(datetime.strptime(start_time, '%d/%m/%y %H:%M:%S')), duration=duration , tutor_id=tutor_id,student_id=student_id)
